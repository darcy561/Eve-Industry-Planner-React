...
- **Collection:** Mongo `user_job_documents` — same [`models.Job`](../../../services/shared/models/job.go) shape; indexes on `(_meta.accountID, displayOnPlanner)` and `(_meta.accountID, groupID)`; `changeStreamPreAndPostImages` listed in [`scripts/bootstrap/mongo-setup.sh`](../../../scripts/bootstrap/mongo-setup.sh).
