...
describe("the offer to build", () => {
  it("says how many rows and what switching them saves", () => {
    render(
      <SourcingOffer summary={summary()} formatIsk={formatIsk} onApply={() => {}} />
    );
  ...
  it("applies when asked", async () => {
    ...
    render(
      <SourcingOffer summary={summary()} formatIsk={formatIsk} onApply={onApply} />
    );
...
describe("the footer", () => {
  it("counts the list and states its volume", () => {
    ...
    expect(screen.getByText("16 materials · 3 buildable · 2 linked")).toBeInTheDocument();
