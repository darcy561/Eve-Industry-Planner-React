...
/**
 * The community store's name for a structure, or null when it has none.
 *
 * The last rung of the ladder: reached only once every linked character has been asked and refused.
 *
 * @param {number} id
 * @returns {Promise<{name: string}|null>}
 */
export function communityName(id) {
  return readCommunityName(id);
}

/**
 * Gives a structure's name back to the community store, for the accounts that cannot read it.
 *
 * Queued rather than sent: naming a page of assets learns many at once, and each is worth a row
 * rather than a request.
 *
 * @param {number} structureId
 * @param {Record<string, unknown>} esi - what ESI answered for the structure
 */
export function submitStructureName(structureId, esi) {
  const submission = buildEsiStructureSubmission(structureId, esi);
  if (!submission) return;

  const idx = submissionQueue.findIndex((e) => e.id === submission.id);
  if (idx >= 0) submissionQueue[idx] = submission;
  else submissionQueue.push(submission);

  scheduleFlush();
}

/**
 * Sends whatever is queued, in requests the API will accept. Safe to call at any time.
 *
 * A chunk that fails puts itself and everything after it back, so nothing is dropped on the way to
 * a store that was briefly unreachable.
 *
 * @returns {Promise<void>}
 */
export async function flushStructureNameSubmissions() {
  ...
  for (let i = 0; i < pending.length; i += MAX_SUBMISSIONS_PER_REQUEST) {
    ...
    const ok = await submitCommunityNames(chunk);
    if (!ok) {
...
function scheduleFlush() {
  ...
  if (submissionQueue.length >= MAX_SUBMISSIONS_BEFORE_FLUSH) {
    void flushStructureNameSubmissions();
    return;
  }
...
// A queue that only ever drains on a timer loses whatever it holds when the page goes away.
