...
func newRotationHarness(t *testing.T) *rotationHarness {
	...
	redislive.Clean(t, client, auth.RefreshTokenKeyPrefix+"*")
	...
	return &rotationHarness{
		handlers: New(&apideps.Deps{Redis: client, EntityCipher: keys.EntityCipher(t)}),
		redis:    client,
		sso:      sso,
	}
}
...
func (h *rotationHarness) seedSession(t *testing.T) string {
	t.Helper()
	now := time.Now().UTC()
	token, err := auth.MintAndStoreRefreshToken(context.Background(), h.redis, auth.RefreshTokenData{
...
func TestPresentingARotatedRefreshTokenIsTerminal(t *testing.T) {
	...
	if code, _ := decodeRotateResponse(t, second)["code"].(string); code != auth.CodeSessionRevoked {
		t.Fatalf("code = %q, want %q — the SPA needs this to stop retrying and start a full login",
			code, auth.CodeSessionRevoked)
	}
}
...
func TestARotateAfterTheReauthDeadlineIsTerminal(t *testing.T) {
	...
	started := time.Now().UTC().Add(-(auth.RefreshTokenTTL + time.Hour))
	token, err := auth.MintAndStoreRefreshToken(context.Background(), h.redis, auth.RefreshTokenData{
		CharacterHash: rotationTestCharacterHsh,
		AccountID:     rotationTestAccountID,
	...
	if code, _ := decodeRotateResponse(t, w)["code"].(string); code != auth.CodeReauthRequired {
		t.Fatalf("code = %q, want %q", code, auth.CodeReauthRequired)
	}
}
...
func TestAnUnknownRefreshTokenIsTerminal(t *testing.T) {
	...
	if code, _ := decodeRotateResponse(t, w)["code"].(string); code != auth.CodeSessionRevoked {
		t.Fatalf("code = %q, want %q", code, auth.CodeSessionRevoked)
	}
}
