...
func TestRespondSessionRefreshTerminalAuthErrorCarriesCode(t *testing.T) {
	...
	respondSessionRefreshTerminalAuthError(w, r, auth.RefreshCredentialLogDetail{},
		auth.CodeSessionRevoked, "planner refresh token not found in Redis",
	...
	if body.Code != auth.CodeSessionRevoked {
		t.Fatalf("code = %q, want %q", body.Code, auth.CodeSessionRevoked)
	}
}
