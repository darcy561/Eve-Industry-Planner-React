...
type Config struct {
	Version     int                    `yaml:"version"` // schema version (always 1 for now)
...
// Proxy is an optional reverse proxy / CDN in front of Traefik.
// Empty TrustedIPs + TrustedCIDRs = Traefik uses the direct peer as the client IP.
type Proxy struct {
...
// ServiceSpec is a capacity/tunable block for one Swarm elastic service.
type ServiceSpec struct {
	CapacityControllerManaged bool    `yaml:"capacity_controller_managed"`
	Min                       int     `yaml:"min"`
	Max                       int     `yaml:"max"`
	Concurrency               int     `yaml:"concurrency"`    // worker
	TargetClients             int     `yaml:"target_clients"` // websocket (soft; controller later)
	ClientCutoff              int     `yaml:"client_cutoff"`  // websocket process refuse + full hint
	ReserveCapacity           float64 `yaml:"reserve_capacity"`
	DrainTimeout              string  `yaml:"drain_timeout"`
}

// LoadYAML reads and unmarshals operator config from path.
...
// Validate fail-closes on missing/invalid capacity fields used by make swarm-sync.
...
func validatePort(field string, n int) error {
	if n == 0 {
		return nil // unset → EffectivePorts default
	}
	if n < 1 || n > 65535 {
...
// EffectiveTrustedProxies returns trimmed proxy.trusted_ips then trusted_cidrs (empty = none).
...
// TrustedProxyCIDRsCSV is Traefik forwardedHeaders.trustedIPs (comma-separated IPs + CIDRs).
func (c Config) TrustedProxyCIDRsCSV() string {
	return strings.Join(c.EffectiveTrustedProxies(), ",")
}

// EffectivePorts returns host publish ports (defaults 80/443/81 when YAML omits/zero).
...
// SyncEnv returns KEY=VALUE lines for stack-deploy interpolation (#32 bridges).
// Order is stable for diffs/tests.
func (c Config) SyncEnv() []string {
	...
	grafanaRoot := "http://127.0.0.1" + grafanaPath + "/"

	// Concurrency / client_cutoff are NOT sync-env bridges — stack ships defaults;
	// make swarm-sync applies YAML via docker service update (apply.go).
...
// SummaryLines are human-readable apply notes for make swarm-sync logs.
func (c Config) SummaryLines() []string {
	...
	lines = append(lines, fmt.Sprintf(
		"ports/paths: http=%d https=%d traefik_dashboard=%d grafana=%q dashboard_path=%q (applied by make swarm-sync)",
	...
	lines = append(lines, "APP_VERSION: SoT is .env (tags/bake/advertise) — not eip.config / swarm-sync")
	return lines
}
