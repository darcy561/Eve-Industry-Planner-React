...
// deleted at DeleteExpiredAfter, which is not. A member who lost access to a long
// ESI outage, or who was away while their token needed re-authorising, rejoins by
...
//
// For the operator view rather than the deletion itself: a number that climbs
// says accounts are losing access somewhere, which a delete count alone would
// hide until DeleteExpiredAfter had passed.
//
// Counts what OwnerKeysForAccount refuses, so the figure cannot disagree with the
// grant it reports on.
func (m *Mongo) CountStaleMemberships(ctx context.Context, now time.Time) (int64, error) {
	...
	err := Retry(ctx, "CountStaleMemberships", func() error {
		got, err := m.PlannerMemberships.Collection().
			CountDocuments(ctx, staleMembershipFilter(now))
		if err != nil {
			return err
		}
		count = got
