...
const reapScratchAccount = "eip-parity-reap-account"

// The gap between StaleAfter and ReapAfter is the point: a row stops granting at
...
func TestLive_reapStaleMemberships_keepsWhatCanStillComeBack(t *testing.T) {
	mongo := mongolive.Require(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()

	account := reapScratchAccount + "-window"
	...
	if _, err := mongo.ReapStaleMemberships(ctx, now); err != nil {
		t.Fatalf("ReapStaleMemberships: %v", err)
	}
...
// Past ReapAfter the row is housekeeping and goes.
// Requires EIP_MONGO_PARITY_LIVE=1.
func TestLive_reapStaleMemberships_deletesWhatWillNot(t *testing.T) {
	mongo := mongolive.Require(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()

	account := reapScratchAccount + "-old"
	corp := models.CorporationOwner(esiCorpRefA)
	cleanupMemberships(t, mongo, account)
	now := time.Now().UTC()
	...
	ageMembership(ctx, t, mongo, corp, account, now.Add(-eipmongo.ReapAfter-time.Hour))

	deleted, err := mongo.ReapStaleMemberships(ctx, now)
	if err != nil {
		t.Fatalf("ReapStaleMemberships: %v", err)
	}
	if deleted < 1 {
	...
	if membershipRows(ctx, t, mongo, corp, account) != 0 {
		t.Error("a row past the reap window survived")
	}
}

// An owner membership has no age at which it should go: nothing outside the
// planner can revoke it, so it never goes stale and the reap must not reach it.
// Requires EIP_MONGO_PARITY_LIVE=1.
func TestLive_reapStaleMemberships_leavesWhatNeverExpires(t *testing.T) {
	mongo := mongolive.Require(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()

	account := reapScratchAccount + "-owner"
	...
	// validate it against.
	old := time.Now().UTC().Add(-eipmongo.ReapAfter - 365*24*time.Hour)
	if err := mongo.EnsureAccountPlanner(ctx, account, old); err != nil {
		t.Fatalf("EnsureAccountPlanner: %v", err)
	}

	if _, err := mongo.ReapStaleMemberships(ctx, time.Now().UTC()); err != nil {
		t.Fatalf("ReapStaleMemberships: %v", err)
	}
	if membershipRows(ctx, t, mongo, owner, account) != 1 {
		t.Error("the reap deleted a membership that never expires")
	}
...
// any cutoff. Reaping on age alone would delete it before a reconcile could ever
// confirm it — so it stops granting, which is correct, and waits.
// Requires EIP_MONGO_PARITY_LIVE=1.
func TestLive_reapStaleMemberships_waitsForARowNeverConfirmed(t *testing.T) {
	mongo := mongolive.Require(t)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()

	account := reapScratchAccount + "-unconfirmed"
	corp := models.CorporationOwner(esiCorpRefA)
	cleanupMemberships(t, mongo, account)
	now := time.Now().UTC()
	...
	if membershipRows(ctx, t, mongo, corp, account) != 1 {
		t.Fatal("a row that was never confirmed was reaped before it could be")
	}
