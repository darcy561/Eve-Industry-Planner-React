...
// The rules an account's record carries with it, wherever it is read or
// written: what a well-formed record looks like, and which of its sessions have
// aged out. They belong with the record rather than with any one way of storing
// it.

func normalizeAccountRecord(rec *AccountRecord, accountID string) {
	if rec == nil {
		return
	}
	...
	if rec.Sessions == nil {
		rec.Sessions = map[string]Session{}
	}
	rec.Grants.OwnerKeys = rec.Grants.OwnerKeys.Normalized()
}

func pruneExpiredSessions(rec *AccountRecord, now time.Time) (removed []string, changed bool) {
