...
// Client-facing auth failure codes. The SPA treats a 401 without a code as retriable, so a
// refusal that can only be cleared by a full EVE SSO login must carry one.
const (
