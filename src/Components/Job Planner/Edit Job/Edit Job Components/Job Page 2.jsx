...
export function EditPage2() {
  ...
  function handleAdd(material) {
    ...
    if (newArray[materialIndex].quantityPurchased >= newArray[materialIndex].quantity) {
  ...
  return (
      <Container>
                  ...
                  <Grid container direction="row">
                    ...
                    {material.purchasing.map((record) => {
                      return (
                          <Grid container direction="row">
                        <Grid xs={2} sm={2} md={1}>
                          <Typography variant="body2">{record.itemCount.toLocaleString()}</Typography>
                        </Grid>
                        <Grid xs={2} sm={2} md={1}>
                          <Typography variant="body2">{record.itemCost.toLocaleString()} ISK</Typography>
                          </Grid>
                          <Grid xs={2} sm={2} md={1}>
                            <IconButton
                              color="primary"
        ...
        <Grid xs={6}>
          <Typography variant="body2">{activeJob.job.products.totalComplete}/{activeJob.job.materials.length} items fully purchased. </Typography>
        </Grid>
        <Grid xs={6}>
          <Typography variant="body2">Total material cost: {activeJob.job.products.totalPurchaseCost.toLocaleString()} ISK</Typography>
          </Grid>
        </Grid>
      </Container>
    );  
