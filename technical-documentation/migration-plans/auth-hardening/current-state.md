...
| #45 | Bootstrap's failure ordering is undocumented | In `services/api/v1endpoints/refresh.go` the Redis writes and the session verify complete before `ResolveUserDocumentsForLogin` is called; what should happen to the freshly minted material when that Mongo read fails has never been decided. |
...
| #52, #53 | Grants failure modes | See § Superseded or owned elsewhere — the premise changed, but a question survives. |
...
| #21, #52, #53 | **Premise changed, by [shared-planners](../shared-planners/plan.md).** A session's grants stopped being a cached list of corporation and alliance ids: Stage B made them `models.SessionGrants` owner keys and Stage C repointed the fill at membership rows, which is why `refresh.go` now calls `mongo.OwnerKeysForAccount` rather than reading a claims cache. That project's Stage E owes the membership revocation path. What survives for this project is the session-side half — see [plan.md](./plan.md) Stage B and Stage E. |
