...
## Reading many buckets shares the ledger walk

`Store.States` reads every bucket the reporter asks for in one pipelined round trip, so a collection
costs one trip rather than two commands per bucket. It reads the ledger with `HGETALL`, the same
command as the single-bucket path, and both hand the fields to one walk that returns spend and the
`esi-sync` share of it.

That sharing is the point. The two paths held separate copies of the walk while the ledger was a
sorted set, and the batch copy was left reading a ZSET when the shape changed — a `WRONGTYPE` error
that would have failed every collection. Nothing caught it: the batch tests read buckets that were
never metered, so they never reached the ledger at all. A metered bucket is now read through both
paths and the figures compared.

## Not yet true anywhere

Nothing in this overlay has promoted. The live SoT still describes the sorted set, and the two
sentences called out above are corrected at promotion, not before.
