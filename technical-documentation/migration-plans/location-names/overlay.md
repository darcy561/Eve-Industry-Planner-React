...
| A batch of lookups | Tested | `getWorldData.test.js` — what resolved kept when a lookup beside it fails, every id a failed batch spoke for reported, no entry written for an unusable answer, the structure-and-public split, and the store-already-holds filter |
