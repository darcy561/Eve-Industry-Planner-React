...
**Depth:** Strong around auth/session helpers and some middleware. Most HTTP handlers and app wiring are untested. Opt-in live Mongo covers the main account Docs call paths handlers use after auth/lock, and the cloud ESI refresh; opt-in live Redis covers the rotate handler end to end.
