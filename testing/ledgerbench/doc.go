...
// argued about.
//
// The ledger scheme is one sorted-set member per call, scored by its own expiry,
// walked in full on every reserve. Its read cost grows with traffic — up to an
// allowance divided by the cheapest charge, which is 12,000 members for a
// 12,000-token bucket met entirely by conditional hits.
//
...
// known set of keys and costs the same whatever the traffic. This is the scheme
// services/shared/esiclient runs.
//
